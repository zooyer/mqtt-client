...
public:
	 ...
	 *  	   complete.
	 */
	deliveryTokenPtr publish(const void* payload, size_t n);
	/**
	 ...
	 *  	   complete.
	 */
	deliveryTokenPtr publish(binary_ref payload);
	/**
	 * Publishes a message on the topic.
	 * @param payload the bytes to use as the message payload
	 * @param qos the Quality of Service to deliver the message at. Valid
	 ...
	 *  	   complete.
	 */
	deliveryTokenPtr publish(binary_ref payload, int qos, bool retained);
	/**
